      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="font-serif text-2xl md:text-3xl lg:text-4xl font-bold text-natural-800 mb-4">
            Perguntas Frequentes de Quem Está <span className="text-natural-600">Decidindo Agora</span>
          </h2>
        ...
        <div className="max-w-3xl mx-auto">
          <div className="space-y-4">
            {faqData.map((item) => (
              ...
              >
                <button
                  className="w-full text-left px-6 py-4 flex justify-between items-center hover:bg-natural-100 transition-colors"
                  onClick={() => toggleItem(item.id)}
                >
                  <span className="font-medium text-natural-800 text-base md:text-lg">❓ {item.question}</span>
                ...
                >
                  <p className="text-natural-700 text-base md:text-lg leading-relaxed">✅ {item.answer}</p>
                </div>
              </div>
            ))}
          </div>
